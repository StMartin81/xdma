 ...
 */
static loff_t
char_sgdma_llseek(struct file* file, loff_t off, int whence)
{
  loff_t newpos = 0;
...
static int
check_transfer_align(struct xdma_engine* engine,
                     ...
                     int sync)
{
  ...
  if (engine->non_incr_addr) {
    ...
    dbg_tfr(
      "buf_lsb = %d, pos_lsb = %d, len_lsb = %ld\n", buf_lsb, pos_lsb, len_lsb);
 ...
 */
static inline void
xdma_io_cb_release(struct xdma_io_cb* cb)
{
  int i;
...
static ssize_t
char_sgdma_read_write(struct file* file,
                      char __user* buf,
                      size_t count,
                      loff_t* pos,
                      bool write)
{
  ...
  res = xdma_xfer_submit(
    xdev, engine->channel, write, *pos, &cb.sgt, 0, sgdma_timeout * 1000);
...
static ssize_t
char_sgdma_write(struct file* file,
                 const char __user* buf,
                 size_t count,
                 loff_t* pos)
{
...
static ssize_t
char_sgdma_read(struct file* file, char __user* buf, size_t count, loff_t* pos)
{
  struct xdma_cdev* xcdev = (struct xdma_cdev*)file->private_data;
  struct xdma_engine* engine;
  int rv;
...
static int
ioctl_do_perf_start(struct xdma_engine* engine, unsigned long arg)
{
  int rv;
  struct xdma_dev* xdev;
  ...
  engine->xdma_perf =
    kzalloc(sizeof(struct xdma_performance_ioctl), GFP_KERNEL);
...
static int
ioctl_do_perf_get(struct xdma_engine* engine, unsigned long arg)
{
  int rc;
...
static int
ioctl_do_addrmode_set(struct xdma_engine* engine, unsigned long arg)
{
  return engine_addrmode_set(engine, arg);
}

static int
ioctl_do_addrmode_get(struct xdma_engine* engine, unsigned long arg)
{
  int rv;
  unsigned long src;
...
static int
ioctl_do_align_get(struct xdma_engine* engine, unsigned long arg)
{
  BUG_ON(!engine);
...
static long
char_sgdma_ioctl(struct file* file, unsigned int cmd, unsigned long arg)
{
  struct xdma_cdev* xcdev = (struct xdma_cdev*)file->private_data;
  struct xdma_dev* xdev;
  struct xdma_engine* engine;
...
static int
char_sgdma_open(struct inode* inode, struct file* file)
{
...
static int
char_sgdma_close(struct inode* inode, struct file* file)
{
  struct xdma_cdev* xcdev = (struct xdma_cdev*)file->private_data;
  struct xdma_engine* engine;
  int rv;
