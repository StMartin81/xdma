...
static const char* const devnode_names[] = {
  XDMA_NODE_NAME "%d_user",          XDMA_NODE_NAME "%d_control",
  XDMA_NODE_NAME "%d_events_%d",     XDMA_NODE_NAME "%d_h2c_%d",
  XDMA_NODE_NAME "%d_c2h_%d",        XDMA_NODE_NAME "%d_bypass_h2c_%d",
  XDMA_NODE_NAME "%d_bypass_c2h_%d", XDMA_NODE_NAME "%d_bypass",
};
...
static inline void
xpdev_flag_set(struct xdma_pci_dev* xpdev, enum xpdev_flags_bits fbit)
{
  xpdev->flags |= 1 << fbit;
}

static inline void
xcdev_flag_clear(struct xdma_pci_dev* xpdev, enum xpdev_flags_bits fbit)
{
  xpdev->flags &= ~(1 << fbit);
}

static inline int
xpdev_flag_test(struct xdma_pci_dev* xpdev, enum xpdev_flags_bits fbit)
{
...
ssize_t
show_device_numbers(struct device* dev,
...
static int
config_kobject(struct xdma_cdev* xcdev, enum cdev_type type)
{
  int rv = -EINVAL;
  ...
  switch (type) {
    ...
    case CHAR_BYPASS_C2H:
      BUG_ON(!engine);
      rv = kobject_set_name(
        &xcdev->cdev.kobj, devnode_names[type], xdev->idx, engine->channel);
      break;
    case CHAR_BYPASS:
    case CHAR_USER:
    case CHAR_CTRL:
    case CHAR_EVENTS:
      rv = kobject_set_name(
        &xcdev->cdev.kobj, devnode_names[type], xdev->idx, xcdev->bar);
      break;
    default:
...
int
xcdev_check(const char* fname, struct xdma_cdev* xcdev, bool check_engine)
{
  ...
  if (check_engine) {
    ...
    if (!engine || engine->magic != MAGIC_ENGINE) {
      pr_info("%s, engine 0x%p, magic 0x%lx.\n",
              fname,
              engine,
              engine ? engine->magic : 0xFFFFFFFF);
      return -EINVAL;
    }
  }
...
static int
create_sys_device(struct xdma_cdev* xcdev, enum cdev_type type)
{
  struct xdma_dev* xdev = xcdev->xdev;
  struct xdma_engine* engine = xcdev->engine;
  int last_param;
  ...
  xcdev->sys_device = device_create(g_xdma_class,
                                    &xdev->pdev->dev,
                                    xcdev->cdevno,
                                    NULL,
                                    devnode_names[type],
...
static int
destroy_xcdev(struct xdma_cdev* cdev)
{
  if (!cdev) {
    pr_warn("cdev NULL.\n");
    return 0;
...
static int
create_xcdev(struct xdma_pci_dev* xpdev,
             struct xdma_cdev* xcdev,
             int bar,
             ...
             enum cdev_type type)
{
  ...
  if (!xpdev->major) {
    ...
    int rv = alloc_chrdev_region(
      &dev, XDMA_MINOR_BASE, XDMA_MINOR_COUNT, XDMA_NODE_NAME);
...
xpdev_destroy_interfaces(struct xdma_pci_dev* xpdev)
{
  ...
  if (xpdev->major)
    unregister_chrdev_region(MKDEV(xpdev->major, XDMA_MINOR_BASE),
                             XDMA_MINOR_COUNT);
}
...
xpdev_create_interfaces(struct xdma_pci_dev* xpdev)
{
  ...
  rv = create_xcdev(
    xpdev, &xpdev->ctrl_cdev, xdev->config_bar_idx, NULL, CHAR_CTRL);
  if (rv < 0) {
  ...
  for (i = 0; i < xpdev->user_max; i++) {
    rv = create_xcdev(xpdev, &xpdev->events_cdev[i], i, NULL, CHAR_EVENTS);
    if (rv < 0) {
  ...
  for (i = 0; i < xpdev->h2c_channel_max; i++) {
    ...
    rv =
      create_xcdev(xpdev, &xpdev->sgdma_h2c_cdev[i], i, engine, CHAR_XDMA_H2C);
    if (rv < 0) {
  ...
  for (i = 0; i < xpdev->c2h_channel_max; i++) {
    ...
    rv =
      create_xcdev(xpdev, &xpdev->sgdma_c2h_cdev[i], i, engine, CHAR_XDMA_C2H);
    if (rv < 0) {
  ...
  if (xdev->bypass_bar_idx > 0) {
    for (i = 0; i < xpdev->h2c_channel_max; i++) {
      ...
      rv = create_xcdev(
        xpdev, &xpdev->bypass_h2c_cdev[i], i, engine, CHAR_BYPASS_H2C);
      if (rv < 0) {
    ...
    for (i = 0; i < xpdev->c2h_channel_max; i++) {
      ...
      rv = create_xcdev(
        xpdev, &xpdev->bypass_c2h_cdev[i], i, engine, CHAR_BYPASS_C2H);
      if (rv < 0) {
    ...
    rv = create_xcdev(
      xpdev, &xpdev->bypass_cdev_base, xdev->bypass_bar_idx, NULL, CHAR_BYPASS);
    if (rv < 0) {
  ...
  if (xdev->user_bar_idx >= 0) {
    rv = create_xcdev(
      xpdev, &xpdev->user_cdev, xdev->user_bar_idx, NULL, CHAR_USER);
    if (rv < 0) {
...
#ifdef __XDMA_SYSFS__
  /* sys file */
  rv = device_create_file(&xpdev->pdev->dev, &dev_attr_xdma_dev_instance);
  if (rv) {
