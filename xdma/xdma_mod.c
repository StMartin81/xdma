...
static void
xpdev_free(struct xdma_pci_dev* xpdev)
{
...
static struct xdma_pci_dev*
xpdev_alloc(struct pci_dev* pdev)
{
...
probe_one(struct pci_dev* pdev, const struct pci_device_id* id)
{
  ...
  hndl = xdma_device_open(DRV_MODULE_NAME,
                          pdev,
                          &xpdev->user_max,
...
static void
xdma_reset_done(struct pci_dev* pdev)
{
  struct xdma_pci_dev* xpdev = dev_get_drvdata(&pdev->dev);
