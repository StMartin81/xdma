 ...
 */
static ssize_t
char_ctrl_read(struct file* fp, char __user* buf, size_t count, loff_t* pos)
{
...
static ssize_t
char_ctrl_write(struct file* file,
...
static long
version_ioctl(struct xdma_cdev* xcdev, void __user* arg)
{
...
long
char_ctrl_ioctl(struct file* filp, unsigned int cmd, unsigned long arg)
{
  ...
  switch (cmd) {
    case XDMA_IOCINFO:
      if (copy_from_user(
            (void*)&ioctl_obj, (void*)arg, sizeof(struct xdma_ioc_base))) {
...
bridge_mmap(struct file* file, struct vm_area_struct* vma)
{
  ...
  dbg_sg("off = 0x%lx\n", off);
  dbg_sg("start = 0x%llx\n",
         (unsigned long long)pci_resource_start(xdev->pdev, xcdev->bar));
  dbg_sg("phys = 0x%lx\n", phys);
  ...
  rv = io_remap_pfn_range(
    vma, vma->vm_start, phys >> PAGE_SHIFT, vsize, vma->vm_page_prot);
